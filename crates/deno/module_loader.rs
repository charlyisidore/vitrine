  ...
  CliModuleLoaderInner<TGraphContainer>
{
  ...
  async fn maybe_reload_dynamic(
    ...
    self
      .shared
      .module_load_preparer
      .reload_specifiers(graph, specifiers_to_reload, true, permissions.clone())
      .await?;
    graph_permit.commit();
    Ok(true)
  }
  fn check_specifiers_to_reload_for_dynamic_import(
    ...
    while let Some((specifier, module_entry)) = module_iter.next() {
      ...
      {
        module_iter.skip_previous_dependencies();
        continue;
      }
      let should_reload = match module_entry {
        ...
        deno_graph::ModuleEntryRef::Err(err) => {
          if matches!(
            ...
            self.mtime_of_specifier(specifier).is_some()
          } else {
  ...
  for CliModuleLoader<TGraphContainer>
{
  ...
  fn prepare_load(
    ...
    async move {
      ...
      let is_dynamic = is_dynamic || inner.is_worker;
      let lib = inner.lib;
