...
impl IntoJs for serde_json::Value {
    fn into_js(self) -> Result<JsValueFacade, JsError> {
        ...
        match self {
            Value::Null => Ok(JsValueFacade::Null),
            Value::Bool(value) => Ok(JsValueFacade::new_bool(value)),
            Value::Number(value) => {
                if value.is_i64() {
            ...
            Value::String(value) => Ok(JsValueFacade::new_string(value)),
            Value::Array(value) => Ok(JsValueFacade::Array {
                val: value
                    .into_iter()
                    .map(IntoJs::into_js)
                    .collect::<Result<_, JsError>>()?,
            }),
            Value::Object(value) => Ok(JsValueFacade::Object {
                val: value
                    .into_iter()
